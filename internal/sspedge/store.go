...
CREATE TABLE IF NOT EXISTS ssp_edge_dispatcher_turns (event_id TEXT PRIMARY KEY,request_ciphertext BLOB NOT NULL,result_ciphertext BLOB,state TEXT NOT NULL CHECK(state IN ('pending','completed')),created_unix_nano INTEGER NOT NULL,expires_unix_nano INTEGER NOT NULL,completed_unix_nano INTEGER);
