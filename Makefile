...
COMMANDS := snagline-control snagline-delivery snagline-edge snagline-front snagline-case snagline-dispatcher snagline-dispatcher-runtime snagline-dispatcher-proxy snagline-buzz-projector snagline-ssp-verify
...
build: sqlcipher-toolchain
	...
	CGO_ENABLED=0 $(GO) build -trimpath -ldflags "$(LDFLAGS)" -o $(BINDIR)/snagline-dispatcher-runtime ./cmd/snagline-dispatcher-runtime
	CGO_ENABLED=0 $(GO) build -trimpath -ldflags "$(LDFLAGS)" -o $(BINDIR)/snagline-dispatcher-proxy ./cmd/snagline-dispatcher-proxy
